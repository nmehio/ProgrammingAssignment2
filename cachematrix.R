...
makeCacheMatrix <- function(x = matrix()){
  #This variable stores the cached matrix.
  m <- NULL
...
cacheSolve <- function(x, ...){
  m <- x$getInverse()
  #This function returns the cached inverse matrix if it is not NULL.
  if(!is.null(m)){
    message("Retrieving cached data.")
    return(m)
  }
  #This block of code computes the inverse matrix if it is not stored in the cache.
  data <- x$getMatrix()
  m <- solve(data)
  x$setInverse(m)
  m
